...
while getopts "iop:st" opt; do
    case $opt in
        ...
        p) device="playerctl"; srce="${OPTARG}"; nsink=$(playerctl --list-all | grep -w -m1 "$srce") ;;
        s) select_output "$(select_output | rofi -dmenu -config "${confDir}/rofi/styles/style_10.rasi")"; exit ;;
...
echo "Device: $device, srce: $srce, nsink: $nsink"
